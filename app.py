...
from environment import (
    EmailTriageEnv, Action, ActionType, EmailCategory, UrgencyLevel,
    TASKS
)
    ...
    return {
        ...
        "action_schema": {
            ...
            "category": {
                ...
                "description": "Email category (use with classify action)"
            },
            "urgency": {
                "type": "string",
                "required": False,
                "values": [u.value for u in UrgencyLevel],
                "description": "Urgency level assigned to the email"
            },
            "reply_text": {
                "type": "string",
                "required": False,
                "description": "Draft reply text (use with draft_reply action)"
            },
            "reason": {
                "type": "string",
                "required": False,
                "description": "Justification for the action (boosts score on hard task)"
            }
        }
    }
...
async def baseline():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return {"error": result.stderr or "Baseline script failed", "output": result.stdout}
    except subprocess.TimeoutExpired:
        return {"error": "Baseline script timed out after 120 seconds"}
    except Exception as e:
        return {"error": str(e)}
